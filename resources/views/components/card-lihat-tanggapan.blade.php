        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="chocolat-parent mb-1">
                    <a href="{{ asset('public/files/' . $pengaduan->foto) }}" class="chocolat-image"
                        title="Gambar Pengaduan" style="width: 200px; height: 200px; object-fit: cover;">
                        <div class="text-center">
                            <img alt="Gambar tidak ditemukan" src="{{ asset('public/files/' . $pengaduan->foto) }}"
                ...
                <b class="text-primary">
                    <i class="fas fa-user"></i> {{ $pengaduan->masyarakat->username }} <i class="bullet"></i> <span
                        class="text-uppercase">{{ $pengaduan->created_at->diffForHumans() }}</span>
                </b>
                <div class="text-dark">
                ...
                @foreach ($tanggapan as $row)
                    ...
                    <div class="mt-1">
                        ...
                        @if ($row->id_masyarakat != null)
                            <a href="{{ route('riwayat.edit', $row->id) }}"
                                class="btn btn-primary float-right ml-1 my-1">
                                <i class="fas fa-edit"></i>
                            </a>

                            <form method="post" action="{{ route('riwayat.destroy', $row->id) }}"
                                id="destroy{{ $row->id }}" class="text-right my-1">
                ...
                <form method="post" action="{{ route('riwayat.store') }}">
                    ...
                    <div class="form-group">
                        <div class="input-group">
                            <textarea name="tanggapan" class="form-control" placeholder="Ketikan tanggapan anda.." @php if($pengaduan->status == 'proses' || $pengaduan->status == 'selesai'){ echo ''; }else{ echo 'readonly'; }  @endphp></textarea>
                            <div class="input-group-append">
                                <button class="btn btn-primary"
                                    type="@php if($pengaduan->status == 'proses' || $pengaduan->status == 'selesai'){ echo 'submit'; }else{ echo 'button'; } @endphp">
