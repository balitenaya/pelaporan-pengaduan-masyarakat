                    ...
                    <div class="chocolat-parent">
                        <a href="{{ asset('public/files/' . $row->foto) }}" class="chocolat-image"
                            ...
                            <div class="text-center">
                                <img alt="Gambar tidak ditemukan" src="{{ asset('public/files/' . $row->foto) }}"
                ...
                <div class="card-footer">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                            <a href="{{ route('pengaduan.verification', $row->id) }}"
                                class="dropdown-item has-icon"><i class="fas fa-check"></i>Verifikasi sekarang</a>
                            <a href="{{ route('verifikasi.tolak', $row->id) }}" class="dropdown-item has-icon"><i
                                    class="fas fa-times"></i>Tolak pengaduan</a>
                            <div class="dropdown-divider"></div>
                            <a href="{{ route('verification.destroy', $row->id) }}"
                                class="dropdown-item has-icon text-danger"><i class="far fa-trash-alt"></i>Hapus</a>
                        </div>
                    </div>
                </div>
            </div>
...
@if ($pengaduan->lastPage() != 1)
    <div class="btn-group float-right">
        ...
        @for ($i = 1; $i <= $pengaduan->lastPage(); $i++)
            <a class="btn btn-primary {{ $i == $pengaduan->currentPage() ? 'active' : '' }}"
                href="{{ $pengaduan->url($i) }}">{{ $i }}</a>
        @endfor
